...
| TC-07 | **`.gitignore` excludes `*.class`** | Compiled bytecode artefacts are excluded from version control by the `.gitignore` file, enforcing a source-only repository. |
...
| OC-03 | **No CI pipeline** | No `.github/workflows` directory is present; builds are manual. A `.github/` directory exists but contains only Copilot agent definitions (`.github/agents/`), hooks, and skills stubs — no automated workflow files. |
| OC-04 | **Copilot agent tooling** | The repository includes GitHub Copilot agent definitions under `.github/agents/`, enabling AI-assisted analysis and documentation generation as an organisational convention. |
...
### 7.4 Version Control Notes

The repository `.gitignore` explicitly excludes `*.class` files:
...
This means:
- Only the source file `HelloWorld.java` is tracked by Git.
- Compiled bytecode (`HelloWorld.class`) is never committed.
- Every deployment environment must perform its own `javac` compilation step from source.
...
### 8.7 Tooling / AI-Assisted Development Concept

The repository adopts a **Copilot-agent-first** approach to documentation and analysis:

| Aspect | Detail |
|--------|--------|
| **Agent definitions** | Stored under `.github/agents/` (e.g., `arc42-documentor.agent.md`, `uml-generator.agent.md`, `ast-analyzer.agent.md`). |
| **Purpose** | Provide AI-generated architecture documentation, UML diagrams, BPMN models, code assessments, and DDL schemas on demand. |
| **Scope** | Currently 11 agent files covering the full software-analysis toolchain. |
| **Execution** | Triggered manually (no CI workflow); agents run via GitHub Copilot in the IDE or API. |
...
### ADR-005 — Adopt GitHub Copilot Agents for Documentation and Analysis

| Field | Value |
|-------|-------|
| **Status** | Accepted |
| **Date** | 2026-03-31 |
| **Context** | The repository is intentionally minimal (one Java source file). Comprehensive architecture documentation, UML diagrams, BPMN models, and code assessments would normally require significant manual effort. |
| **Decision** | Store Copilot agent definitions under `.github/agents/` to automate all documentation and analysis tasks via AI. |
| **Rationale** | AI-assisted tooling can generate Arc42 documentation, class diagrams, sequence diagrams, business process models, and quality assessments from source code, reducing manual overhead to near zero even for future growth scenarios. |
| **Consequences** | Documentation quality depends on agent prompt quality and model capability. Agents must be maintained alongside the codebase. No automated trigger exists; agents are run on-demand. |
| **Alternatives considered** | Manual documentation (too time-consuming), Javadoc only (insufficient for architecture-level documentation). |
...
| R-06 | **Agent-generated documentation drift** — Copilot agent outputs may become stale if source code changes and agents are not re-run. | Medium | Low | Documentation | Schedule agent re-runs as part of any code change review process; consider linking agent execution to a CI trigger. |
...
| TD-06 | Add CI trigger to auto-run Copilot agents on code changes | 30 min | Low |
...
gantt
    ...
    section Tooling
    CI trigger for Copilot agents    :td06, after td03, 1d
```
...
| **Copilot Agent** | A GitHub Copilot customisation defined in a `.md` file under `.github/agents/`. Specifies a specialised AI persona and task (e.g., arc42 documentation, UML generation) to be executed on demand. |
...
| **Mermaid** | A JavaScript-based diagramming and charting tool that uses Markdown-inspired text definitions to create diagrams. Used throughout this document for all architecture diagrams. |
...
*Based on source analysis of `HelloWorld.java`, `README.md`, `.gitignore`, and `.github/agents/` in repository `copilot-test-ktruchcz`.*  
